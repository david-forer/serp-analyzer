...
class OutputWriter:
    ...
    def write_csv(self, results: List[Dict[str, Any]], filename: str = None, 
        ...
        fieldnames = [
            ...
            'result_type', 'page_type', 'brand_tier', 'result_confidence',
            # Content strategy columns (query-level)
            'competition', 'recommended_content_type', 'recommended_format', 'recommended_angle',
        ...
        with open(filepath, 'w', newline='', encoding='utf-8') as f:
            ...
            for result in results:
                ...
                row = {
                    ...
                    'brand_tier': result.get('classification', {}).get('brand_tier', ''),
                    'result_confidence': result.get('classification', {}).get('confidence', ''),
                    # Content strategy (same for all rows)
                    'competition': (f"{strategy_rec.get('competition', {}).get('level', '')}: "
                                    f"{strategy_rec.get('competition', {}).get('summary', '')}").strip(': '),
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            ...
            if 'content_strategy' in analysis:
                ...
                if rec.get('competition'):
                    ...
                    f.write(f"COMPETITION:\n  {comp.get('level', '')}. {comp.get('summary', '')}\n\n")
                if rec.get('format_warning'):
                    f.write(f"HEADS UP:\n  {rec['format_warning']}\n\n")
